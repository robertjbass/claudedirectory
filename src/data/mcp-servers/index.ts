...
// Generated by `scripts/ingest/awesome-mcp.mjs`. Curated entries above
// always win on slug collision.
import ingestedMcpServersRaw from "@/data/_ingested/mcp-servers.json";

const curatedMcpServerSlugs = new Set(curatedMcpServers.map((s) => s.slug));
const ingestedMcpServers: MCPServer[] = (ingestedMcpServersRaw as unknown as MCPServer[]).filter(
  (s) => s && s.slug && !curatedMcpServerSlugs.has(s.slug),
);

export const mcpServers: MCPServer[] = [...curatedMcpServers, ...ingestedMcpServers];
