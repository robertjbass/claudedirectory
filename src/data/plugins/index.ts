...
// known marketplaces. Generated by `scripts/ingest/marketplaces.mjs`.
// Curated entries above always win on slug collision.
import ingestedPluginsRaw from "@/data/_ingested/plugins.json";

const curatedPluginSlugs = new Set(curatedPlugins.map((p) => p.slug));
const ingestedPlugins: Plugin[] = (ingestedPluginsRaw as unknown as Plugin[]).filter(
  (p) => p && p.slug && !curatedPluginSlugs.has(p.slug),
);

export const plugins: Plugin[] = [...curatedPlugins, ...ingestedPlugins];
